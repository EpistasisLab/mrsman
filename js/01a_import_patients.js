...
var formatPatient = function(record) {
    var gender = record.PatientGender;
    ...
    var birthdate = record.PatientDateOfBirth.toISOString();
    ...
    var person = {
        ...
        //format gender between source and destination db
        "gender": gender.substring(0, 1),
        "birthdate": birthdate,
    }
    var identifiers = [{
        "identifierType": "8d79403a-c2cc-11de-8d13-0010c6dffd0f", //identifies this as an "Old OpenMRS ID" type 
   ...
   "identifierType": "05a29f94-c0ed-11e2-94be-8c13b969e334", // OpenMRS ID
    "identifier": OpenMRSID, // valid Luhn mod 30 check digit
    "location": "8d6c993e-c2cc-11de-8d13-0010c6dffd0f" // Default Location
    }];
    var patient = {
        "person": person,
...
//get the number of records to be imported
// group  = null or a hex value from 0-f
var getNum = function(group) {
    var sql = 'SELECT count(*) num from Patients where PatientID not in (select PatientID from OpenMRSPatients)'
    if (group) {
        sql+=' and RIGHT(MD5(PatientId),1) = "'+group+'"';
    }
    db.query(sql).then(
        function handleSettled(snapshots) {
            total = snapshots[0][0].num;
            log.log(total + ' records to import');
            if(total == 0) {
               log.log('no records to import');
               pool.end()
            } else {
               log.log('start import');
               importBatch(group);
            }
        }
    );
