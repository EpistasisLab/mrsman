...
insert into concepts (itemid,shortname,longname,concept_type,linksto) select itemid,label,concat(label,' [',concept_type,'_',itemid,']'),concat('test_',concept_type),'labevents' linksto from (select itemid,label,unnest('{text,num}'::text[]) concept_type from d_labitems) c;
...
create temporary table lenum_tmp_1 as select itemid,valueuom,count(*) from labevents where valuenum is not null group by itemid,valueuom order by itemid;
-- summarize values
drop table if exists lenum_tmp_2;
create temporary table lenum_tmp_2 as select max(valuenum) max_val,min(valuenum) min_val,avg(valuenum) avg_val,itemid,count(*) from labevents where valuenum is not null group by itemid order by itemid,count desc;
